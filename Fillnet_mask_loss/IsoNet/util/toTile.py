...
class reform3D:
    ...
    def pad_and_crop_new(self, cubesize=32, cropsize = 64): #
        ...
        data = np.pad(self._orig_data,((padi,padsize[0]),(padi,padsize[1]),(padi,padsize[2]),(0,0)),'symmetric')#矩阵
        outdata=[]

        for i in range(self._sidelen[0]):#就是将我要复原的3维数据集切成小块分别在x，y，z轴上切割
    ...
    def restore_from_cubes_new(self,cubes, cubesize = 32, cropsize = 64):#将小切片3维图像 变成大的3维图像
        if len(cubes.shape)==5 and cubes.shape[-1]==1:
            cubes = cubes.reshape(cubes.shape[0:-1])
        ...
        return new[0:self._sp[0],0:self._sp[1],0:self._sp[2]]#随后还是取以前的shape数据
