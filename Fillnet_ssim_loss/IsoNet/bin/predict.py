...
def predict(args):
    ...
    args.gpuID = str(args.gpuID) #获取GPU id
    args.ngpus = len(list(set(args.gpuID.split(',')))) #获取GPU数量
    ...
    if args.batch_size is None:
        args.batch_size = 4 * args.ngpus #max(4, 2 * args.ngpus) #这里因为batch_size 可以被gpu平分 所以到每个gpu数量是 batch_size/gpu数量
    #print('batch_size',args.batch_size)
    os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"
    os.environ["CUDA_VISIBLE_DEVICES"]=args.gpuID
    #check gpu settings
    from IsoNet.bin.refine import check_gpu
    check_gpu(args)#查看GPU是否可用
    ...
    if not os.path.isdir(args.output_dir):#创建corrected_tomos文件
    ...
    for it in md:
        if args.tomo_idx is None or str(it.rlnIndex) in args.tomo_idx:
            ...
            if os.path.isfile(tomo_file):
                ...
                predict_one(args,tomo_file,output_file=tomo_out_name) #开始修复3维矩阵
